<x-dashboard-layout title="Edit Campaign">
    ...
    <div class="max-w-3xl mx-auto" x-data="campaignWizard()">
        @if($campaign->rejection_reason && $campaign->status->value === 'draft')
            <div class="mb-6 p-4 bg-red-50 border border-red-200">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-sm font-semibold text-red-800">Campaign Rejected — Needs Changes</p>
                        <p class="text-sm text-red-700 mt-1">{{ $campaign->rejection_reason }}</p>
                        <p class="text-xs text-red-500 mt-2">Please make the necessary changes and resubmit for review.</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        <div class="bg-white border border-gray-200 overflow-hidden">
            <div class="p-6 sm:p-8">
                <form action="{{ route('campaigns.update', $campaign) }}" method="POST" enctype="multipart/form-data" x-ref="form">
                    @csrf
                    @method('PUT')
                            ...
                            <div>
                                @if($campaign->status->value === 'draft')
                                    <button type="button" onclick="document.getElementById('publish-form').submit()"
                                            class="px-6 py-2.5 bg-[#CE5F26] text-white text-sm font-semibold hover:bg-[#B04E1E] transition shadow-lg shadow-[#CE5F26]/30">
                ...
                @if($campaign->status->value === 'draft')
                    <form id="publish-form" action="{{ route('campaigns.publish', $campaign) }}" method="POST" class="hidden">
                        @csrf
                    </form>
                @endif
            </div>
