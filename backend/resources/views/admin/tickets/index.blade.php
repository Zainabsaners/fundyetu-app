<x-admin-layout title="Support Tickets">
    ...
    <div class="bg-white border border-gray-200 overflow-hidden">
        <div class="overflow-x-auto">
            <table id="adminTicketsTable" class="w-full text-sm border-collapse">
                <thead>
                    <tr class="bg-gray-50/50">
                        ...
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider border border-gray-200">Subject</th>
                        ...
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider border border-gray-200">Submitted</th>
                ...
                <tbody>
                    @foreach($tickets as $ticket)
                        <tr class="hover:bg-gray-50/50 transition">
                            ...
                            <td class="px-4 py-3 border border-gray-200 text-gray-800 max-w-xs truncate">{{ $ticket->subject }}</td>
                            <td class="px-4 py-3 text-center border border-gray-200">
                                <span class="text-xs font-medium px-2 py-0.5
                                    @if($ticket->status === 'open') bg-yellow-50 text-yellow-700
                                    @else bg-green-50 text-green-700 @endif">
                                    {{ ucfirst($ticket->status) }}
                                </span>
                            </td>
                            <td class="px-4 py-3 border border-gray-200 text-xs text-gray-500 whitespace-nowrap">{{ $ticket->created_at->format('d M Y g:i A') }}</td>
                            <td class="px-4 py-3 text-left border border-gray-200 whitespace-nowrap">
                                <div class="flex items-center justify-start gap-1.5">
                                    <a href="{{ route('admin.tickets.show', $ticket) }}" class="text-[11px] font-medium px-2 py-0.5 text-white rounded transition" style="background-color: #1B2A4A;">View</a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
